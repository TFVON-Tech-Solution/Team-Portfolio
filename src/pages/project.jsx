...
const images = [
  {
    img: alumniSystem,
    title: 'The Alumni Management System for CVSU-Imus tracks graduates employment and professional development through an online platform, helping improve curriculum and generate reports.',
    video: alumniDemo,
  },
  {
    img: payrollSystem,
    title: 'exPense is a cloud-based payroll system that automates salary calculations, tax deductions, and bank transfers. It supports various pay structures and provides employees with ePayslips via a mobile app.',
    video: payrollDemo,
  },
  {
    img: businessProject,
    title: 'Craves On the Go is an ordering and inventory system that helps manage pastry sales, track stock, and handle customer orders efficiently.',
    video: businessDemo,
  },
  {
    img: hotelSystem,
    title: 'The Shangri-la Hotel Management System is a system designed to streamline hotel bookings, manage guest info, and improve overall hotel operations.',
    video: hotelDemo,
  },
  {
    img: librarySystem,
    title: 'Libra Sphere is a system that helps libraries manage books, users, and borrowing activities efficiently.',
    video: libraryDemo,
  },
];
...
const Project = () => {
  ...
  const [activeIndex, setActiveIndex] = useState(null); // For mobile detail view
  ...
  const handleProjectClick = (index, video) => {
    const isMobile = window.innerWidth <= 768;

    if (isMobile) {
      if (activeIndex === index) {
        openModal(video); // 2nd click: open video
        setActiveIndex(null); // reset
      } else {
        setActiveIndex(index); // 1st click: show detail
      }
    } else {
      openModal(video); // Desktop behavior: show video immediately
    }
  };
        ...
        {images.map((item, index) => (
          <React.Fragment key={index}>
            <div
              ...
              onClick={() => handleProjectClick(index, item.video)}
            >
              <img
                src={item.img}
                alt={`Project ${index + 1}`}
              ...
              <div className="hidden sm:flex absolute inset-0 bg-gradient-to-t from-black via-gray-800 to-transparent rounded-xl items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span className="text-white text-[15px] font-bold text-center px-4">{item.title}</span>
              </div>
              {/* Mobile detail description */}
              <div className={`sm:hidden absolute inset-0 bg-black bg-opacity-70 rounded-xl flex flex-col items-center justify-center px-4 transition-all duration-300 ${activeIndex === index ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <span className="text-white text-[15px] font-bold text-center">{item.title}</span>
